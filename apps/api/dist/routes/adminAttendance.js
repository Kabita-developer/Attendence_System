...
router.get("/", asyncHandler(async (req, res) => {
    ...
    const attendance = Array.from(byDate.entries()).map(([date, records]) => {
        ...
        return {
            ...
            employeeId: records[0].userId?.employeeId || "",
            employeeName: records[0].userId?.name || "",
            slots: records.map((a) => ({
                id: String(a._id),
...
router.post("/:id/approve", asyncHandler(async (req, res) => {
    ...
    if (!attendance)
        return res.status(404).json({ ok: false, error: { message: "Attendance not found", status: 404 } });
    ...
    attendance.reviewedBy = req.auth.userId;
...
router.post("/upsert", asyncHandler(async (req, res) => {
    ...
    const updated = await AttendanceModel.findOneAndUpdate({ userId: user._id, attendanceDate, slotId: body.slotId }, {
        $set: {
...
router.post("/clear", asyncHandler(async (req, res) => {
    ...
    const match = { userId: user._id, attendanceDate };
    if (body.slotId) {
