...
router.post("/mark", requireRole("EMPLOYEE"), asyncHandler(async (req, res) => {
    const body = z
        .object({
    ...
    let slot = cache.get(`slot:${body.slotId}`);
    if (!slot) {
...
router.get("/me", requireRole("EMPLOYEE"), asyncHandler(async (req, res) => {
    const query = z
        .object({
