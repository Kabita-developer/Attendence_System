...
router.post("/change-password", requireRole("ADMIN"), // Only admins can change passwords
asyncHandler(async (req, res) => {
    ...
    if (!ok)
        return res.status(400).json({ ok: false, error: { message: "Invalid current password", status: 400 } });
...
router.post("/admin/request-password-reset", asyncHandler(async (req, res) => {
    ...
    if (!admin)
        return res.json({ ok: true, data: { requested: true } });
...
router.post("/admin/confirm-password-reset", asyncHandler(async (req, res) => {
    const body = z
        .object({
        email: z.string().email(),
        otp: z.string().min(4),
