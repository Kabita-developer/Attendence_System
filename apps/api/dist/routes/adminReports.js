...
function monthSalaryRows(opts) {
    ...
    for (const a of opts.attendance) {
        ...
        if (a.status === "APPROVED") {
            ...
            byUser[key].pendingSlots += 1;
        else if (a.status === "REJECTED")
            byUser[key].rejectedSlots += 1;
    }
...
router.get("/daily.pdf", asyncHandler(async (req, res) => {
    const q = z.object({ date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/) }).parse(req.query);
    const day = DateTime.fromISO(q.date, { zone: APP_TZ }).startOf("day");
...
router.get("/daily.xlsx", asyncHandler(async (req, res) => {
    const q = z.object({ date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/) }).parse(req.query);
    const day = DateTime.fromISO(q.date, { zone: APP_TZ }).startOf("day");
    const employees = await UserModel.find({ role: "EMPLOYEE", isActive: true }).sort({ employeeId: 1 }).lean();
    const attendance = await AttendanceModel.find({ attendanceDate: day.toJSDate() })
...
router.get("/monthly-salary", asyncHandler(async (req, res) => {
...
router.get("/monthly-salary.xlsx", asyncHandler(async (req, res) => {
    const q = z.object({ month: z.string().regex(/^\d{4}-\d{2}$/) }).parse(req.query);
    const start = DateTime.fromFormat(q.month, "yyyy-MM", { zone: APP_TZ }).startOf("month");
    const end = start.endOf("month");
    const employees = await UserModel.find({ role: "EMPLOYEE", isActive: true }).sort({ employeeId: 1 }).lean();
...
router.get("/monthly-salary.pdf", asyncHandler(async (req, res) => {
    const q = z.object({ month: z.string().regex(/^\d{4}-\d{2}$/) }).parse(req.query);
    const start = DateTime.fromFormat(q.month, "yyyy-MM", { zone: APP_TZ }).startOf("month");
    const end = start.endOf("month");
    const employees = await UserModel.find({ role: "EMPLOYEE", isActive: true }).sort({ employeeId: 1 }).lean();
...
router.get("/employee-summary", asyncHandler(async (req, res) => {
    const q = z
        .object({
    ...
    const start = DateTime.fromFormat(q.month, "yyyy-MM", { zone: APP_TZ }).startOf("month");
    const end = start.endOf("month");
    const user = (await UserModel.findOne({ role: "EMPLOYEE", employeeId: q.employeeId }).lean());
    if (!user)
...
router.get("/employee-summary.xlsx", asyncHandler(async (req, res) => {
    const q = z
        .object({
        month: z.string().regex(/^\d{4}-\d{2}$/),
        employeeId: z.string().min(3)
...
router.get("/employee-summary.pdf", asyncHandler(async (req, res) => {
    const q = z
        .object({
        month: z.string().regex(/^\d{4}-\d{2}$/),
        employeeId: z.string().min(3)
    ...
    res.setHeader("content-type", "application/pdf");
    res.setHeader("content-disposition", `attachment; filename="employee-summary-${user.employeeId}-${q.month}.pdf"`);
    const doc = new PDFDocument({ margin: 36 });
    doc.pipe(res);
